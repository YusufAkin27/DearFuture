...
public interface ContractAcceptanceRepository extends JpaRepository<ContractAcceptance, Long> {
     ...
     */
    List<ContractAcceptance> findByUserOrderByAcceptedAtDesc(User user);
    ...
    @Query("SELECT ca FROM ContractAcceptance ca " +
           ...
           "     LOWER(CONCAT(COALESCE(ca.user.firstName,''), ' ', COALESCE(ca.user.lastName,''))) LIKE LOWER(CONCAT('%', :searchTerm, '%')))) OR " +
